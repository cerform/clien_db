...
class GoogleCalendarSync:
    ...
    def _load_credentials(self):
        ...
        if self.credentials_file and os.path.exists(self.credentials_file):
            logger.info(f"🔍 Loading calendar credentials from file: {self.credentials_file}...")
            try:
                credentials = Credentials.from_service_account_file(
                    self.credentials_file,
                    scopes=SCOPES
                )
                logger.info(f"✅ Successfully loaded calendar credentials from file")
                return credentials
            except Exception as e:
                logger.error(f"❌ Failed to load calendar credentials from file: {e}")
                raise
        
        # Otherwise use Cloud Run's native Service Account
        logger.info("🔍 Loading calendar credentials using google.auth.default()...")
        
        try:
            credentials, project = google_auth_default(scopes=SCOPES)
            logger.info(f"✅ Successfully loaded calendar credentials from environment")
            return credentials
        except Exception as e:
