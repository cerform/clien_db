    ...
    <body class="blackwork-theme">
        <div class="container dark">
            <div class="header dark">
                <h1 class="page-head-icon"><img data-icon="master" src="/static/icons/icon-master.svg" alt="masters"/> Управление Мастерами</h1>
            ...
            <div class="content">
                ...
                <div class="search-box">
                    ...
                    <button class="btn btn--primary" onclick="openAddModal()">➕ Добавить мастера</button>
                </div>
                
                <div class="table-container card dark" style="padding:12px;">
            ...
            function renderMasters(masters) {
                ...
                tbody.innerHTML = masters.map(m => {
                    ...
                    return `<tr>
                        ...
                        <td>
                            <button class="btn-edit" onclick="editMaster('${jsEscape(m.id)}')">✏️ Редактировать</button>
                            <button class="btn-delete" onclick="deleteMaster('${jsEscape(m.id)}', '${jsEscape(m.name)}')">🗑️ Удалить</button>
                        </td>
                    </tr>`;
                }).join('');
            }
...
async def db_manager_page():
    """Database Manager page for admin panel (dark blackwork theme)."""
    return """
    <!DOCTYPE html>
    <html lang="ru">
    <head>
        ...
        <style>
            ...
            .card-list .card-item + .card-item { margin-top: 8px; }
            .action-btn { min-width: 40px; text-align: center; }
        </style>
    </head>
    <body class="blackwork-theme">
        <div class="container dark">
            <div class="header dark">
                ...
                <div class="control-row">
                    <button class="btn btn--muted" onclick="window.location.href='/'">← Back</button>
                    <button class="btn btn--primary" onclick="loadSheets()">🔃 Load Sheets</button>
                    <button class="btn btn--muted" onclick="backupDB()">💾 Backup</button>
                    <button class="btn btn--danger" onclick="restoreDB()">♻️ Restore (replace)</button>
                    <input type="file" id="uploadBackup" style="display:none" accept="application/json" onchange="uploadBackup(event)" />
                </div>
            </div>
            <div class="card dark legacy-expose" style="margin:18px 18px;">
                <div style="display:flex;justify-content:space-between;align-items:center;">
                    <h3 style="margin:0">Sheets</h3>
                    <div class="search-box control-row">
                        <input id="sheetSearch" placeholder="Search sheets or tables..." oninput="filterSheets()" />
                    </div>
                </div>
                <div id="sheetsList" class="card-list" style="margin-top:12px">Loading sheets...</div>
            </div>
            <div class="card dark legacy-expose" style="margin:18px;">
                <div style="display:flex;justify-content:space-between;align-items:center;">
                    <h3 style="margin:0">Audit Logs</h3>
                    <div>
                        <button class="btn btn--primary" onclick="loadAuditLogs()">Load Audit Logs</button>
                    </div>
                </div>
                <pre id="auditLogs">No logs</pre>
            </div>
        </div>
        <!-- Modal for viewing sheets -->
        <div id="sheetModal" class="modal">
            <div class="modal-content">
                <div class="modal-header">
                    <h2>Sheet Preview</h2>
                    <button class="close" onclick="closeSheetModal()">&times;</button>
                </div>
                <pre id="sheetPreview">Loading...</pre>
            </div>
        </div>
        <script>
            async function loadSheets() {
                try {
                    ...
                    if (!sheets || sheets.length == 0) {
                        el.innerHTML = '<div style="padding:12px;color:var(--muted)">No sheets found</div>';
                        return;
                    }
                    el.innerHTML = sheets.map(s => `
                        <div class='card-item'>
                            <div style='flex:1'>
                                <strong>${s}</strong>
                                <div class='footer-small' style='margin-top:6px'>${s.length > 50 ? s.substring(0,50)+"..." : ""}</div>
                            </div>
                            <div style='display:flex;gap:8px'>
                                <button class='action-btn' title='View' onclick="viewSheet('${jsEscape(s)}')">👁️</button>
                                <button class='action-btn' title='Export' onclick="exportSheet('${jsEscape(s)}')">⬇️</button>
                                <button class='action-btn' title='Import (append)' onclick="document.getElementById('upload_${encodeURIComponent(s)}').click()">⬆️</button>
            ...
            async function viewSheet(name) {
                ...
                document.getElementById('sheetPreview').textContent = JSON.stringify(data, null, 2);
        ...
        <script>
            ...
            // Simple filter for sheets
            function filterSheets() {
                const q = (document.getElementById('sheetSearch').value || '').toLowerCase();
                const items = document.querySelectorAll('#sheetsList .card-item');
                items.forEach(it => {
                    const title = it.querySelector('strong').textContent.toLowerCase();
...
async def services_page():
    ...
    <body class="blackwork-theme">
        <div class="container dark">
            <div class="header dark">
                <h1 class="page-head-icon"><img data-icon="services" src="/static/icons/icon-services.svg" alt="services"/> Управление Услугами</h1>
                <button class="btn-back" onclick="window.location.href='/'">← Назад</button>
            </div>
            <div class="content">
                <div id="alert" class="alert"></div>
                ...
                <div class="search-box">
                    ...
                    <button class="btn btn--primary" onclick="openAddModal()">➕ Добавить услугу</button>
                </div>
            ...
            function renderServices(services) {
                ...
                tbody.innerHTML = services.map(s => {
                    ...
                    return `<tr>
                        ...
                        <td class="${statusClass}">${statusText}</td>
                        <td>
                            <button class="btn-edit" onclick="editService('${jsEscape(s.id)}')">✏️</button>
                            <button class="btn-delete" onclick="deleteService('${jsEscape(s.id)}', '${jsEscape(s.name)}')">🗑️</button>
                        </td>
                    </tr>`;
                }).join('');
            }
...
async def clients_page():
    ...
    <body class="blackwork-theme">
        <div class="container dark">
            <div class="header dark">
                <h1 class="page-head-icon"><img data-icon="clients" src="/static/icons/icon-clients.svg" alt="clients"/> Управление Клиентами</h1>
                <button class="btn-back" onclick="window.location.href='/'">← Назад</button>
            </div>
            <div class="content">
                <div id="alert" class="alert"></div>
                ...
                <div class="search-box">
                    ...
                    <button class="btn btn--primary" onclick="openAddModal()">➕ Добавить клиента</button>
                    <button class="btn btn--muted" onclick="resetFilter()" style="background:#6c757d;">Показать все</button>
                </div>
        ...
        <script>
            ...
            function renderClients(clients) {
                ...
                tbody.innerHTML = clients.map(c => {
                    ...
                    let actions = '';
                    if (isDeleted) {
                        actions = `<button class="btn-edit" onclick="restoreClient('${jsEscape(c.id)}')">♻️ Восстановить</button>`;
                    } else {
                        actions = `<button class="btn-view" onclick="viewClient('${jsEscape(c.id)}')">👁️ Подробнее</button>
                                   <button class="btn-edit" onclick="editClient('${jsEscape(c.id)}')">✏️</button>
                                   <button class="btn-delete" onclick="deleteClient('${jsEscape(c.id)}', '${jsEscape(c.name)}')">🗑️</button>`;
                    }
            ...
            async function deleteClient(id, name) {
                ...
                try {
                    const token = localStorage.getItem('admin_token');
                    const headers = token ? { 'Authorization': 'Bearer ' + token } : {};
                    const response = await fetch(`/api/clients/${id}`, { method: 'DELETE', headers });
                    const result = await response.json();
...
async def bookings_page():
    ...
    <body class="blackwork-theme">
        <div class="container dark">
            <div class="header dark">
                <h1 class="page-head-icon"><img data-icon="bookings" src="/static/icons/icon-bookings.svg" alt="bookings"/> Управление Записями</h1>
                <button class="btn-back" onclick="window.location.href='/'">← Назад</button>
            </div>
            <div class="content">
                <div id="alert" class="alert"></div>
        ...
        <script>
            ...
            function renderBookings(bookings) {
                ...
                tbody.innerHTML = bookings.map(b => {
                    ...
                    let actions = '';
                    if (b.status === 'pending') {
                        actions = `<button class="action-btn btn-confirm" onclick="confirmBooking('${jsEscape(b.id)}')">✅ Подтвердить</button>
                                   <button class="action-btn btn-cancel" onclick="openCancelModal('${jsEscape(b.id)}')">❌ Отменить</button>`;
                    } else if (b.status === 'confirmed') {
                        actions = `<button class="action-btn btn-complete" onclick="completeBooking('${jsEscape(b.id)}')">✓ Выполнено</button>
                                   <button class="action-btn btn-cancel" onclick="openCancelModal('${jsEscape(b.id)}')">❌ Отменить</button>`;
                    }
...
async def inka_training_page():
    ...
    <body class="blackwork-theme">
        <div class="container dark">
            <div class="header dark">
                <h1 class="page-head-icon"><img data-icon="inka" src="/static/icons/icon-inka.svg" alt="inka"/> Обучение ИНКИ</h1>
                <button class="btn-back" onclick="window.location.href='/'">← Назад</button>
            </div>
            <div class="content">
        ...
        <script>
            ...
            window.showTab = function showTab(tab) {
                ...
                const caller = (typeof event !== 'undefined' && event && event.target) ? event.target : document.activeElement;
                try { caller.classList.add('active'); } catch(e) {}
            ...
            async function loadStats() {
                try {
                    ...
                    const data = await res.json();
                    document.getElementById('totalSessions').textContent = data.total_examples || 0;
                    document.getElementById('totalKnowledge').textContent = Object.keys(data.categories || {}).length;
                    document.getElementById('totalCorrections').textContent = data.total_improvements || 0;
                    
                    // Обновить отображение категорий
                    if (data.categories) {
                        const catText = Object.entries(data.categories)
                            .map(([cat, count]) => `${cat}: ${count}`)
                            .join(', ');
                        const catEl = document.getElementById('categoriesBreakdown');
                        if (catEl) catEl.textContent = catText || 'Нет данных';
                    }
                } catch (e) {
                ...
                try {
                    ...
                    container.innerHTML = data.scenarios.map(s => `
                        <div class="knowledge-item">
                            <div>
                                <span class="knowledge-type">${s.category || 'general'}</span>
                                <strong>${(s.user_input || '').substring(0, 50)}...</strong>
                                <p style="color:#666;font-size:12px;margin:5px 0 0 0">${(s.inka_response || '').substring(0, 80)}...</p>
                                <small style="color:#999">${s.timestamp || ''}</small>
                            </div>
                            <button class="delete-btn" onclick="deleteScenario('${jsEscape(s.id)}')">🗑️</button>
                        </div>
                    `).join('');
                } catch (e) {
                        ...
                        <div class="knowledge-item">
                            <div>
                                <div style="margin-bottom:8px">
                                    <strong>Контекст:</strong> ${(c.user_input || '').substring(0, 60)}...
                                </div>
                                <div style="color:#dc3545;margin-bottom:4px">
                                    <strong>✗ Неправильно:</strong> ${(c.inka_response || '').substring(0, 50)}...
                                </div>
                                <div style="color:#28a745">
                                    <strong>✓ Правильно:</strong> ${(c.admin_correction || '').substring(0, 50)}...
                                </div>
                                <small style="color:#999">${c.timestamp || ''}</small>
                            </div>
                            <button class="delete-btn" onclick="deleteCorrection('${jsEscape(c.id)}')">🗑️</button>
                        </div>
                    `).join('');
                } catch (e) {
            ...
            async function loadKnowledge() {
                try {
                    ...
                    container.innerHTML = data.knowledge.map(k => `
                        <div class="knowledge-item">
                            <div>
                                <span class="knowledge-type">${k.category || 'general'}</span>
                                <strong>${(k.user_input || '').substring(0, 40)}...</strong>
                                <p style="color:#666;font-size:12px;margin:5px 0 0 0">${(k.inka_response || '').substring(0, 100)}...</p>
                                <small style="color:#999">${k.timestamp || ''}</small>
                                ${k.tags ? '<div style="margin-top:4px;font-size:11px;color:#007bff">' + k.tags + '</div>' : ''}
                            </div>
                            <button class="delete-btn" onclick="deleteKnowledge('${jsEscape(k.id)}')">🗑️</button>
                        </div>
                    `).join('');
                } catch (e) {
                ...
                try {
                    ...
                    container.innerHTML = data.recent.slice(0, 5).map(r => `
                        <div style="padding:5px 0;border-bottom:1px solid #eee">
                            <strong>${r.category || 'general'}</strong>: ${(r.user_input || '').substring(0, 40)}...<br>
                            <small style="color:#666">${(r.inka_response || '').substring(0, 60)}...</small><br>
...
async def analytics_page():
    ...
    <body class="blackwork-theme">
        <div class="container dark">
            <div class="header dark">
                <h1 class="page-head-icon"><img data-icon="analytics" src="/static/icons/icon-analytics.svg" alt="analytics"/> Аналитика</h1>
                <button class="btn-back" onclick="window.location.href='/'">← Назад</button>
            </div>
            <div class="content">
    ...
    <head>
        ...
        <style>
            * { margin: 0; padding: 0; box-sizing: border-box; }
            ...
            .master-chip .calendar-status.error { color: #ff9800; }
        ...
        <script>
            ...
            // Set to Monday
                        function jsEscape(val) {
                            try {
                                // Use JSON.stringify to escape special characters, then strip surrounding quotes
                                const s = JSON.stringify(String(val || ''));
                                // Remove surrounding double quotes and escape single quotes for single-quoted attributes
            ...
            async function loadMasters() {
                try {
                    ...
                    if (!res.ok) {
                        ...
                        document.getElementById('masterSelector').innerHTML = `<p style="color:red">Ошибка загрузки мастеров: ${res.status}</p>`;
                        console.error('Failed to load masters:', res.status, err);
                        return;
                    }
                    ...
                    document.getElementById('masterSelector').innerHTML = masters
                        ...
                        .map(m => `
                            <div class="master-chip ${currentMasterId === m.id ? 'active' : ''}" data-id="${jsEscape(m.id)}" onclick="selectMaster('${jsEscape(m.id)}', event)">
                                ${m.name}
                                <span class="calendar-status ${m.calendar_id ? 'connected' : 'disconnected'}" id="master_status_${m.id}">
                                    ${m.calendar_id ? '●' : '○'}
                                </span>
                            </div>
                    ...
                    try { sendTelemetry({event_type: 'ui_error', message: 'Error loading masters', meta: {error: (e && e.message) || String(e)}}); } catch(e2) {}

                    // Show specific error message
                    let errorMsg = 'Ошибка загрузки мастеров';
                    if (e.name === 'AbortError') {
                        errorMsg = 'Превышено время ожидания. Проверьте соединение.';
                    } else if (e.message) {
                        errorMsg += ': ' + e.message;
                    }
                    document.getElementById('masterSelector').innerHTML = `<p style="color:red">${errorMsg}</p><button onclick="loadMasters()" style="margin-top:10px;padding:8px 16px;cursor:pointer;background:#667eea;color:white;border:none;border-radius:5px">Попробовать снова</button>`;
                }
            }
            ...
            window.showTab = function showTab(tab) {
                ...
                // support being called without an explicit event (inline onclick may not always provide it)
                const caller = (typeof event !== 'undefined' && event && event.target) ? event.target : document.activeElement;
                try { caller.classList.add('active'); } catch (e) {}
            ...
            async function loadSyncStatus() {
                ...
                try {
                    ...
                    const res = await fetch(`/api/calendar/sync/${masterId}`, { headers });
                    ...
                    document.getElementById('calendarStatus').innerHTML = (calendar_is_connected && calendar_ok_to_fetch)
                        ? `<p style="color:green">✅ Подключен</p>
                           <p style="font-size:12px;color:#666">ID: ${data.calendar_id ? data.calendar_id.substring(0, 30) : ''}...</p>
            ...
            function renderScheduleTable(schedule) {
                ...
                tbody.innerHTML = schedule.map(s => {
                    const isWorking = String(s.is_working || '').toLowerCase() === 'true';
                    return `<tr style="background:${isWorking ? '#e8f5e9' : '#ffebee'}">
                        ...
                        <td style="padding:12px;text-align:center">
                            <button onclick="editScheduleEntry('${jsEscape(s.id)}')" style="padding:5px 10px;cursor:pointer;background:#ffc107;border:none;border-radius:3px" title="Редактировать">✏️</button>
                            <button onclick="toggleWorkDay('${jsEscape(s.id)}', ${!isWorking})" style="padding:5px 10px;cursor:pointer;background:${isWorking ? '#dc3545' : '#28a745'};border:none;border-radius:3px;color:white" title="${isWorking ? 'Отключить' : 'Включить'}">${isWorking ? '🚫' : '✅'}</button>
                        </td>
                    </tr>`;
                }).join('');
            }
            ...
            function editScheduleEntry(entryId) {
                ...
                document.getElementById('edit_is_working').checked = String(entry.is_working || '').toLowerCase() === 'true';
            ...
            window.prevWeek = function prevWeek() {
            ...
            window.goToToday = function goToToday() {
                currentWeekStart = new Date();
                currentWeekStart.setDate(currentWeekStart.getDate() - currentWeekStart.getDay() + 1);
                loadSalonCalendar();
            }
            
            window.prevWeekMaster = function prevWeekMaster() {
                masterWeekStart.setDate(masterWeekStart.getDate() - 7);
                loadMasterCalendar();
            }
            
            window.nextWeekMaster = function nextWeekMaster() {
                masterWeekStart.setDate(masterWeekStart.getDate() + 7);
                loadMasterCalendar();
            }
            
            window.loadSalonCalendar = async function loadSalonCalendar() {
                ...
                try {
                    ...
                    const [bookingsRes, mastersRes, clientsRes, servicesRes, salonCalRes] = await Promise.all([
                        fetch('/api/bookings', { headers, signal: controller.signal }),
                        fetch('/api/masters', { headers, signal: controller.signal }),
                        fetch('/api/clients', { headers, signal: controller.signal }),
                        fetch('/api/services', { headers, signal: controller.signal }),
                        // Salon calendar events
                        fetch(`/api/calendar/salon?start_date=${startDate}&end_date=${endDate}`, { headers, signal: controller.signal }).catch(e => null)
                    ]);
                    clearTimeout(timeoutId);
                    ...
                    console.log('loadSalonCalendar: bookings=', bookings.length, 'masters=', activeMasters.length);
                    const days = ['Пн', 'Вт', 'Ср', 'Чт', 'Пт', 'Сб', 'Вс'];
                    let salonCalendarEvents = [];
                    try {
                        if (salonCalRes && salonCalRes.ok) {
                            const salonJson = await salonCalRes.json();
                            if (salonJson && salonJson.success && Array.isArray(salonJson.calendar_events)) {
                                salonCalendarEvents = salonJson.calendar_events;
                            }
                            console.log('Loaded salon calendar events:', salonCalendarEvents.length);
                        }
                    } catch(e) { console.warn('Failed to load salon calendar events', e); }
                    ...
                    for (const master of activeMasters) {
                        ...
                        for (let i = 0; i < 7; i++) {
                            ...
                            if (dayBookings.length === 0 && salonCalendarEvents.filter(e => {
                                const d = (e.start && (e.start.date || (e.start.dateTime || '').substring(0,10))) || '';
                                return d === dateStr;
                            }).length === 0) {
                                html += `<div class="timeline-event" style="background:#e0e0e0;color:#999">${days[i]} - свободно</div>`;
                            } else {
                                // Render salon events for this date first
                                const daySalonEvents = salonCalendarEvents.filter(e => {
                                    const d = (e.start && (e.start.date || (e.start.dateTime || '').substring(0,10))) || '';
                                    return d === dateStr;
                                });
                                for (const evt of daySalonEvents) {
                                    html += `<div class="timeline-event" style="background:#6aa0ff;color:white" onclick="showEventDetails('${jsEscape(evt.id)}', '${jsEscape(evt.summary || '')}', '', '${dateStr}', '${(evt.start && (evt.start.dateTime || evt.start.date) || '').substring(11,16)}', 'event', '')">${days[i]} ${((evt.start && (evt.start.dateTime || evt.start.date) || '').substring(11,16)) || ''}<br>${(evt.summary || 'Salon event')}</div>`;
                                }
                                ...
                                for (const b of dayBookings) {
                                    ...
                                    html += `
                                        <div class="timeline-event" style="background:${statusColors[b.status] || '#667eea'}"
                                             onclick="showEventDetails('${jsEscape(b.id)}', '${jsEscape((client && client.name) || '')}', '${jsEscape((service && service.name) || '')}', '${jsEscape(b.date)}', '${jsEscape(b.time)}', '${jsEscape(b.status)}', '${jsEscape(b.price || 0)}')">
                                            ${days[i]} ${b.time || ''}<br>${((client && client.name && client.name.split(' ')[0]) || 'Клиент')}
                                        </div>
                                    `;
                                }
                            }
                    ...
                    document.getElementById('salonCalendar').innerHTML = `<p style="color:red;text-align:center">${errorMsg}</p><button onclick="loadSalonCalendar()" style="margin-top:10px;padding:8px 16px;cursor:pointer;background:#667eea;color:white;border:none;border-radius:5px">Попробовать снова</button>`;
                }
            }
            
            window.selectMaster = function selectMaster(masterId, e) {
                currentMasterId = masterId;
                document.querySelectorAll('.master-chip').forEach(c => c.classList.remove('active'));
                try {
                    if (e && e.target) {
                        const chip = e.target.closest('.master-chip');
                        if (chip) chip.classList.add('active');
                    } else {
                        const chip = document.querySelector(`.master-chip[data-id="${masterId}"]`);
                        if (chip) chip.classList.add('active');
                    }
                } catch(e) {}
                loadMasterCalendar();
            ...
            window.loadMasterCalendar = async function loadMasterCalendar() {
                ...
                try {
                    ...
                    const res = await fetch(`/api/calendar/master/${currentMasterId}?start_date=${startDate}&end_date=${endDate}`, { headers });
                    if (!res.ok) {
                            ...
                            for (const evt of slotEvents) {
                                ...
                                html += `
                                    <div class="event ${statusClass}" 
                                         onclick="showEventDetails('${jsEscape(evt.id)}', '${jsEscape(evt.client_name || evt.title)}', '${jsEscape(evt.service_name || '')}', '${jsEscape(dateStr)}', '${jsEscape(hour)}', '${jsEscape(evt.status || 'event')}', '${jsEscape(evt.price || '')}')">
                    ...
                    if (data.calendar_id && !data.calendar_fetch_ok) {
                        ...
                        // Update status badge on master chip according to calendar fetch
                        try {
                            const statusEl = document.getElementById('master_status_' + currentMasterId);
                            if (statusEl) {
                                if (data.calendar_id && data.calendar_fetch_ok) {
                                    statusEl.className = 'calendar-status connected';
                                    statusEl.innerHTML = '●';
                                } else if (data.calendar_id && !data.calendar_fetch_ok) {
            ...
            window.checkAvailability = async function checkAvailability() {
                ...
                try {
                    ...
                    const res = await fetch(`/api/calendar/availability/${masterId}?date=${date}`, { headers });
                    if (!res.ok) {
                        if (res.status === 401 || res.status === 403) {
                            window.location.href = '/login';
                            return;
                        }
                        document.getElementById('availabilityGrid').innerHTML = '<p style="color:red">Ошибка проверки доступности</p>';
                        const errTxt = await res.text();
                        console.error('Availability fetch failed', res.status, errTxt);
                        return;
                    }
                    const data = await res.json();
                    ...
                    document.getElementById('availabilityGrid').innerHTML = allSlots.map(slot => {
                        ...
                        return `
                            <div class="time-slot-btn ${isAvailable ? 'available' : 'busy'}"
                                 ${isAvailable ? `onclick="bookSlot('${jsEscape(masterId)}', '${jsEscape(date)}', '${jsEscape(slot)}')"` : ''}>
                                ${slot}
            ...
            window.openAddEventModal = function openAddEventModal() {
                if (!currentMasterId) {
            ...
            window.showEventDetails = function showEventDetails(id, clientName, serviceName, date, time, status, price) {
                const statusNames = {
                ...
                document.getElementById('eventDetailsContent').innerHTML = `
                    ...
                    ${typeof id === 'string' && id.startsWith && id.startsWith('booking_') ? `
            ...
            try {
                ...
                if (availDateEl) availDateEl.value = formatDate(new Date());
            } catch(e) { console.warn('Init: availDate not set', e); }

            try { console.log('schedule: script start'); } catch(e) {}
            try { loadMasters(); } catch(e) { console.error('loadMasters failed', e); }
            try { loadSalonCalendar(); } catch(e) { console.error('loadSalonCalendar failed', e); }
            try { console.log('schedule: script end'); } catch(e) {}
            try { console.log('Functions: prevWeek=', typeof window.prevWeek, 'nextWeek=', typeof window.nextWeek, 'goToToday=', typeof window.goToToday, 'showTab=', typeof window.showTab, 'loadSalonCalendar=', typeof window.loadSalonCalendar); } catch(e) {}
        </script>
    </body>
    </html>
    """
...
async def admins_page():
    ...
    <body class="blackwork-theme">
        <div class="container dark">
            <div class="header">
                <h1 class="page-head-icon"><img data-icon="admin" src="/static/icons/icon-admin.svg" alt="admins"/> Управление Админами</h1>
                <button class="btn-back" onclick="window.location.href='/'">← Назад</button>
            </div>
        ...
        <script>
            ...
            function renderAdmins(admins) {
                ...
                tbody.innerHTML = admins.map(a => {
                    ...
                    return `<tr>
                        ...
                        <td class="${statusClass}">${statusText}</td>
                        <td>
                            <button class="btn-edit" onclick="editAdmin('${jsEscape(a.id)}')" title="Редактировать">✏️</button>
                            <button class="btn-password" onclick="openPasswordModal('${jsEscape(a.id)}', '${jsEscape(a.username)}')" title="Изменить пароль">🔐</button>
                            <button class="btn-toggle ${toggleClass}" onclick="toggleStatus('${jsEscape(a.id)}')" title="${a.is_active ? 'Деактивировать' : 'Активировать'}">${toggleText}</button>
                            <button class="btn-delete" onclick="deleteAdmin('${jsEscape(a.id)}', '${jsEscape(a.username)}')" title="Удалить">🗑️</button>
                        </td>
                    </tr>`;
                }).join('');
            }
            ...
            // Load data on page load — ensure we don't use `return` at top-level
            const token = localStorage.getItem('admin_token');
            if (!token) {
                window.location.href = '/login';
            } else {
...
async def telemetry_page():
    ...
    <body class="blackwork-theme">
        <div class="container dark" style="padding:20px;">
...
async def monitoring_page():
    ...
    with open("src/web/templates/admin_monitoring.html", "r", encoding="utf-8") as f:
        return f.read()


@admin_router.get("/database", response_class=HTMLResponse)
async def database_page():
    """Excel-like database management interface"""
    # Read the HTML template file
    with open("src/web/templates/admin_database.html", "r", encoding="utf-8") as f:
        return f.read()
